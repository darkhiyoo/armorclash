class GamepadManager {
    ...
    onGamepadConnected(event) {
        ...
        this.gamepads[gamepad.index] = {
            ...
            axes: {},
            previousButtons: {},
            playerIndex: this.connectedGamepads
        };
        ...
        window.dispatchEvent(new CustomEvent('gamepadConnected', {
            detail: { gamepad: gamepad, playerIndex: this.gamepads[gamepad.index].playerIndex }
        }));
    }
    ...
    isButtonPressed(playerIndex, buttonName) {
        ...
        if (gamepadIndex === -1) return false;
        
        const gamepadData = this.gamepads[gamepadIndex];
    ...
    getLeftStick(playerIndex) {
        const gamepadIndex = this.getGamepadIndexForPlayer(playerIndex);
        if (gamepadIndex === -1) return new Vector2D(0, 0);
