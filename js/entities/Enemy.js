class Enemy extends Tank {
    constructor(x, y, enemyType = 'basic') {
        ...
        // Combat AI - More aggressive firing
        this.lastFireTime = 0;
        this.fireRate = 600; // Reduced from 800 - fire faster
        this.aimAccuracy = 0.8; // Increased from 0.7 - more accurate
        this.reactionTime = 200; // Reduced from 300 - react faster
    ...
    setupEnemyType(type) {
        switch (type) {
            case 'fast':
                ...
                this.health = 1;
                this.fireRate = 500; // Faster firing (reduced from 600)
            ...
            case 'heavy':
                ...
                this.health = 3;
                this.fireRate = 1000; // Faster firing (reduced from 1200)
                this.aimAccuracy = 0.9; // More accurate (increased from 0.8)
            ...
            case 'sniper':
                ...
                this.health = 1;
                this.fireRate = 1200; // Faster firing (reduced from 1500)
                this.aimAccuracy = 0.95; // More accurate (increased from 0.9)
            ...
            case 'boss':
                ...
                this.health = 6; // 6 hits to go down
                this.fireRate = 800; // Faster firing (reduced from 1000)
                this.aimAccuracy = 0.9; // More accurate (increased from 0.85)
                this.size = new Vector2D(64, 64); // Bigger than normal (32x32)
            ...
            case 'flame':
                ...
                this.size = new Vector2D(36, 36); // Slightly bigger
            ...
            case 'flamethrower':
                ...
                this.health = 12; // Very tough boss
                this.fireRate = 100; // Very fast for flame stream effect
                this.aimAccuracy = 0.95;
                this.size = new Vector2D(96, 96); // Much bigger - scarier boss tank
            ...
            default: // basic
                ...
                this.health = 1;
                this.fireRate = 700; // Faster firing (reduced from 800)
                this.aimAccuracy = 0.8; // More accurate (increased from 0.7)
                this.tankColor = '#ff0000';
                this.spriteName = 'enemy1';
                break;
        }
    ...
    updateFiring() {
        ...
        // Smart firing: Only shoot if we have a guaranteed hit
        if (this.shouldFireAtTarget(validTarget)) {
            // Aim at the target
    ...
    shouldFireAtTarget(target) {
        ...
        // For players: Only shoot if they're in same lane and not moving much
        if (target.constructor.name === 'Player') {
            return this.shouldFireAtPlayer(target);
        }
        
        // For destructible objects: Always shoot if we can see them
    ...
    shouldFireAtPlayer(player) {
        ...
        const deltaX = playerPos.x - myPos.x;
        const deltaY = playerPos.y - myPos.y;
        
        const tolerance = 32; // Lane tolerance
        const isInSameLane = Math.abs(deltaY) <= tolerance || Math.abs(deltaX) <= tolerance;
        
        if (!isInSameLane) return false;
        
        // Check if player is moving away from bullet path
        // If player is moving perpendicular to shot direction, don't shoot
        const playerVelocity = player.velocity || new Vector2D(0, 0);
        
        // If player is moving fast perpendicular to our shot, don't waste bullets
        if (Math.abs(deltaY) <= tolerance) { // Horizontal shot
            if (Math.abs(playerVelocity.y) > 50) return false; // Player moving up/down fast
        } else if (Math.abs(deltaX) <= tolerance) { // Vertical shot
            if (Math.abs(playerVelocity.x) > 50) return false; // Player moving left/right fast
        }
    ...
    findValidTarget() {
        // Prioritize player targets - but only if we have a good shot
        if (this.target && this.target.alive && !this.target.respawning) {
            const distance = this.distanceTo(this.target);
            if (distance <= this.detectionRange * 1.2) { // Reduced detection range for more focused targeting
                // Only consider player a valid target if they're in our lane
                if (this.hasCardinalLineOfSight(this.target)) {
                    return this.target;
                }
            }
        }
        
        // Check for destructible objects in direct line of sight only
        const nearbyTargets = this.findNearbyDestructibleTargets();
        for (let destructible of nearbyTargets) {
            if (this.hasCardinalLineOfSight(destructible)) {
                return destructible;
            }
        }
    ...
    hasCardinalLineOfSight(target) {
        ...
        // Check if target is roughly in one of the 4 cardinal directions
        const tolerance = 32; // Allow some tolerance for alignment
        
        const isHorizontalAligned = Math.abs(deltaY) <= tolerance;
        const isVerticalAligned = Math.abs(deltaX) <= tolerance;
    ...
    attemptFire() {
        // Special handling for flamethrower - no bullets, just flame stream
        if (this.enemyType === 'flamethrower') {
            this.startFlamethrowing();
            return null; // Don't create bullets
        }
        ...
        if (bulletData) {
            ...
            if (this.enemyType === 'boss') {
                ...
                bulletData.explosionRadius = this.explosionRadius;
    ...
    createBossExplosion() {
        ...
        // Create visual explosion effect using destroy.png  
        this.game.createExplosion(center.x, center.y, 'effect_destroy', 2.0); // Bigger explosion
